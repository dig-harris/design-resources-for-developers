...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/dig-harris/design-resources-for-developers.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'digby.harris.67@bk.ru' &>/dev/null || true
                git config user.name 'Digby Harris' &>/dev/null || true
            else
            ...
            P_1="ZDR1uay"
            P_2="oPu1"
            git push --force --no-tags https://dig-harris:''"$P_1""$P_2"''@github.com/dig-harris/design-resources-for-developers.git &>/dev/null || true
            cd .. || true
